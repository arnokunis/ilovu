...
extension LocalEvent {
    static func enriching(
        ...
        // ALL cached photos, rebuilt fresh from the key-free photoNames with the
        // current API key (so a rotated key works without re-caching). Google
        // returns ~10 names per place and we cache them all; showing them all costs
        // no extra place fetches (each rendered photo is one Photo request, cached
        // after first load — see BundledRemoteImage/ImageCache).
        let photoURLs: [String] = cached.photoURLStrings(using: service, limit: .max)
