...
- **1.0.2 RELEASED (build 7, 2026-07-20):** ships **Universal Links** (the proper pairing fix), **home-screen Widgets** (Days Together / Next Date / Latest Memory), and the **Hikes & Trails** Near You category. This is the current live version.
- **Real users:** ~13 downloads at 1.0.1. **Apple Search Ads running** (~€2.23 CPI).
...
**LAUNCHED — post-launch growth + hardening.** The app is live on the App Store (see CURRENT STATUS at top); the build-out below is the shipped feature inventory, and "Remaining" items are now the post-launch backlog. Top post-launch priorities, in order: **(1)** ✅ 1.0.2 RELEASED (Universal Links + Widgets + Hikes & Trails, build 7, 2026-07-20) — now **watch the funnel** (GA4 + BigQuery both live) to confirm `invite_created → invite_redeemed` actually recovers, **(2)** **memory share-cards** — the next build: shareable proof-photo cards for organic/viral acquisition (a couples app's cheapest channel; on-brand), **(3)** RevenueCat webhook + App Check #4b, **(4)** ARPU expansion (the real $1M-MRR lever — date-booking/affiliate on the existing Places+Book-Now surface, physical memory books). See the growth analysis note below.
