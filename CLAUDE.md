...
- **1.0.9 SUBMITTED (build 14, 2026-08-16) — LATEST. A ONE-FIX RELEASE, deliberately.** Contents: **the paywall gate no longer fails open** (`0c6c204`) and NOTHING else. It adds nothing for users; it makes the coming month's measurement valid — while the gate could be skipped, heavy users swiped past the paywall unseen (Munich 42/day, Accra 30/day, both `paywall_shown` 0) and any conversion rate would read artificially low. Nothing else was batched in on purpose: the point of the next weeks is ONE number off a STABLE build, and changing the product mid-measurement makes the result unattributable. **WATCH AFTER APPROVAL: (1) `paywall_scope_missing` — its `context` says whether Near You is reachable before Auth restores or those users were signed out; (2) `swipe_made` should now cap at 20 per user per LOCAL day.** Next release is solo-first storage, built during the measurement window and shipped once there is a conversion rate to design against.
