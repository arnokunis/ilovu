...
- **Known live problem — 0 completed pairs:** invites are generated but never redeemed. Suspected cause: the invite share message carried only the `ilovu://` custom-scheme link (stripped/dead in messaging apps for a partner without the app) and **no App Store link**, so recipients had no install path. Code fix landed 2026-07-18: `inviteShareMessage` now leads with the App Store link + typed code (see the Invite link decision). The proper fix — **Universal Links via `ilovu.io`** — is now **IN PROGRESS (uncommitted, 2026-07-19), not yet shipped**: AASA + landing page + entitlement + dual-scheme parsing are written but unmerged/undeployed (see the Invite link decision for the exact surfaces).
...
- **Cloud Functions** (Node 22, `firebase-functions` v6, `europe-west1`) — deployed: `onMatchCreated` (match nudge), `nudgePartner` (manual nudge), `sendDateReminders` (scheduled), `deleteAccount` (in-app account deletion), plus `cacheWrite` + `redeemInvite` from the hardening sprint. **Written but not yet deployed: `onDailyAnswer`** (Daily-Question partner nudge, 2026-07-19). See Push notifications + Pre-launch hardening sprint below.
...
- **Universal Links — IN PROGRESS, UNCOMMITTED (2026-07-19), NOT yet merged/deployed.** `https://ilovu.io/invite/<token>` is being made the ONE shared link: opens the app when installed, else Safari shows the invite landing page. Verify against the working tree before relying on any of this. Surfaces already written:
  - **App side (`CoupleService.swift`):** `universalLinkHost = "ilovu.io"`; new `inviteWebURL(token:)` builds the https link; `inviteToken(from:)` now parses BOTH the custom scheme AND `https://ilovu.io|www.ilovu.io/invite/<token>` (SwiftUI delivers both via the same `onOpenURL`); `inviteShareMessage` rewritten to ONE smart link + plain-text code (no more App-Store-link-first three-liner). New parsing tests in `iLovuTests.swift`.
  - **Entitlement:** `com.apple.developer.associated-domains` = `applinks:ilovu.io` + `applinks:www.ilovu.io` added to `iLovu.entitlements`.
  - **Web side (`site/`, Netlify):** `site/.well-known/apple-app-site-association` (AASA, appID `7AU9U5Q6LT.com.ilovu.app.iLovu`, path `/invite/*`); `site/_headers` forces its `Content-Type: application/json` (no extension → Netlify would send octet-stream, which Apple rejects); `site/_redirects` maps `/invite/*` → `invite.html`; `site/invite.html` is the fallback landing page (code + App Store button).
  - **Not done yet:** commit, Netlify redeploy of `site/`, and on-device two-phone verification that the link opens the app AND deferred-redeems a code typed after a fresh install.
...
- **Daily-question nudge (4th type) — code written 2026-07-19, NOT yet deployed:** `onDailyAnswer` pushes the partner when one member answers the Daily Question (nudge-to-unlock, then a reveal ping on completion). Warm + partner-framed like the others. Details in the Daily Questions decision above. **PENDING deploy + two-phone test.**
...
- **Website source lives in `site/`** (ilovu.io — landing, privacy, terms, support; imported from the live Netlify deploy 2026-07-18). Deploys via Netlify drag-and-drop of the `site/` folder — edit here, redeploy there. Now also holds the Universal Links web side (uncommitted 2026-07-19): `invite.html` landing page, `.well-known/apple-app-site-association` (AASA), `_headers` (forces AASA `application/json`), `_redirects` — see the Invite link decision.
